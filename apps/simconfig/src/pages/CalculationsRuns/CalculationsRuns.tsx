...
import { useGetCalculationRunListQuery, useGetModelFileListQuery } from '@cognite/simconfig-api-sdk/rtk';
import type { CalculationRun, GetCalculationRunListApiArg } from '@cognite/simconfig-api-sdk/rtk';
...
export function CalculationRuns(){
	...
	const preAppliedFilters: Partial<GetCalculationRunListApiArg> = useSearch<AppLocationGenerics>();
	const {
    data: { definitions },
		...
		if(preAppliedFilters.calculationType && preAppliedFilters.modelName){
			setSelectedOptions(options => ({
				...
				calculationType: {
					...
					value: { key: 'calculationType', value: preAppliedFilters.calculationType ?? ''}
				}
			}))
		}
	}, [preAppliedFilters])
	...
	return (
		<CalculationRunsContainer onScroll={handleScroll}>
			<CalculationsFilterContainer>
				...
				<AutoComplete
					options={generateOptions('calculationType', 'Calculations', definitions?.type.calculation)}
				...
				<AutoComplete
					options={generateOptions('calculationRunType', 'Run Type', definitions?.calculation.runType)}
				...
				<AutoComplete
					options={
						generateOptions('calculationRunStatus', 'Status', definitions?.calculation.runStatus)
					}
