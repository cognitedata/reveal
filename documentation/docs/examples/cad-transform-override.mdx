...
Note that any transformation override is applied with respect to the THREE.js world coordinate space.
...
model.getBoundingBoxByTreeIndex(533010).then(boundingBox => {
  ...
  const intervalID = setInterval(function () {
    ...
    const newRotation = new THREE.Euler(0, -Math.PI / 300 * counter, 0);
