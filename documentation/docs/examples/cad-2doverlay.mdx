---
id: cad-2doverlay
title: HTML overlays
description: Demo shows how to add an HTML overlay over 3D objects.
---

import { DemoWrapper } from '@site/src/components/DemoWrapper';
import useBaseUrl from '@docusaurus/useBaseUrl';

A common requirement for a 3D application is to add information overlays hovering over 3D objects
to provide details or draw the attention to a given point. This can be done by placing HTML elements over the 3D canvas
that follows the object it's attached to. Note that these overlays always will be placed on top of the 3D viewer,
they will not be hidden when the object is obscured by other objects.
To achieve this it's necessary to use custom 3D objects.

:::note
There should be a limited number of HTML overlays as it can impact performance negatively if there's too many.
:::

<DemoWrapper name="Cognite3DViewerDemo" />

Attaching HTML elements to 3D objects/positions requires the following operations:

1. Create a HTML element and apply style `position: absolute` to the element. There also some other recommended 
   styles that should be applied (see below).
2. Determine 3D world coordinate of the pivot point (e.g. using intersection when user clicks an object)
3. Attach overlay using `Cognite3DViewer.attachHtmlOverlay`.

This will attach the overlay to the 2D position corresponding to the 3D coordinate provide and update
the coordinate whenever navigating the viewer.

There might be "any" number of attached HTML overlays in a viewer, but avoid too many to avoid cluttering the
view.

```jsx runnable
function createOverlay() {
  const element = document.createElement('div');
  element.innerText = `Overlay!`;
  element.style.cssText = `
    position: absolute;
    
    /* Anchor to the center of the element and ignore events */
    transform: translate(-50%, -50%);
    pointer-events: none;
    touch-action: none;

    /* Make it look nice */
    padding: 10px;
    minHeight: 50px;
    color: #fff;
    background: #232323da;
    borderRadius: 0.25em;
    border: '#ffffff22 solid 2px;
  `;
  return element;
}

// Attach overlay on click
const overlays = viewer.createHtmlOverlayTool();
viewer.on('click', event => {
  // Find 3D coordinate of what the user clicks
  const intersection = viewer.getIntersectionFromPixel(event.offsetX, event.offsetY);
  if (intersection) {
    const el = createOverlay();
    overlays.add(el, intersection.point);
    // Remove overlay after 5 seconds
    setTimeout(() => overlays.remove(el), 5000);
  }
});
```

It's also possible apply styling based on the position of the 
overlay, e.g. to vary the opacity of the overlay based on distance 
from the camera. To do this, provide a `options`-argument with a 
`positionUpdatedCallback` to `Cognite3DViewer.attachHtmlOverlay`. This
callback is triggered every time the HTML overlay is updated.

```js runnable
function createOverlay() {
  const element = document.createElement('div');
  element.innerText = `Overlay!`;
  element.style.cssText = `
    position: absolute;
    
    /* Anchor to the center of the element and ignore events */
    transform: translate(-50%, -50%);
    pointer-events: none;
    touch-action: none;

    /* Make it look nice */
    padding: 10px;
    color: #fff;
    background: #232323da;
    borderRadius: 0.25em;
    border: '#ffffff22 solid 2px;
  `;
  return element;
}

const overlays = viewer.createHtmlOverlayTool();
viewer.on('click', event => {
  // Find 3D coordinate of what the user clicks
  const intersection = viewer.getIntersectionFromPixel(event.offsetX, event.offsetY);
  if (intersection) {
    const el = createOverlay();
    const options = {
      positionUpdatedCallback: (element, position2D, position3D, distanceToCamera) => {
        // Compute opacity based on distance to camera
        const opacity = 1.0 - (Math.min(Math.max(distanceToCamera, 10), 110) - 10) / 100.0;
        element.style.opacity = opacity;
      }
    };
    overlays.add(el, intersection.point, options);
  }
});
```

As shown above, overlays can be removed using `remove()`. It is also possible to remove all 
overlays by disposing the tool:

```js runnable
function createOverlay() {
  const element = document.createElement('div');
  element.innerText = `!`;
  element.style.cssText = `
    position: absolute;
    
    /* Anchor to the center of the element and ignore events */
    transform: translate(-50%, -50%);
    pointer-events: none;
    touch-action: none;

    /* Make it look nice */
    padding: 10px;
    minHeight: 50px;
    color: #000000;
    background: #f5a614;
    borderRadius: 0.25em;
    border: '#ffffff22 solid 2px;
  `;
  return element;
}

const overlays = viewer.createHtmlOverlayTool();
const bounds = model.getModelBoundingBox();
for (let i = 0; i < 100; i++) {
  const s = [Math.random(), Math.random(), Math.random()];
  const pos = new THREE.Vector3(
      bounds.min.x + (bounds.max.x - bounds.min.x) * s[0],
      bounds.min.y + (bounds.max.y - bounds.min.y) * s[1],
      bounds.min.z + (bounds.max.z - bounds.min.z) * s[2]
    );
  overlays.add(createOverlay(), pos);
}

// Dispose all overlays after 10 seconds
setTimeout(() => overlays.dispose(), 10000);
```
