 ...
 * }
 */
export const DeleteAnnotationsAndHandleLinkedAssetsOfFileV1 = createAsyncThunk<
  void,
  {
  ...
  ThunkConfig
>(
  'DeleteAnnotationsAndRemoveLinkedAssetsV1',
  async ({ annotationIds, showWarnings }, { getState, dispatch }) => {
    ...
    if (verifiedAssetRefAnnotations.length) {
      ...
      ) as Map<number, VisionAnnotationV1[]>; // HACK: cast to V1 type since verifiedAssetRefAnnotations has type VisionAnnotationV1
