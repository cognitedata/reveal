...
## Events
The `Image360Collection` exposes events `image360Entered` and `image360Exited` for subscribing to enter & exit from 360 image mode. `image360Entered` is also fired when the revision of an image is changed.
...
## Historical images
Some image collections include 360 images with historical revisions. You can use the `360Image` interface to manage which revision of the image is displayed. 
To get a list of all available revisions for a 360 image, use the `getRevisions` method. The method returns a sorted list of `Image360Revision` objects by date, 
with the most recent revision at the beginning of the list.
...
```ts
const onImageEntered: Image360EnteredDelegate = (entity: Image360, revision: Image360Revision) => {
  entity.list360ImageRevisions.forEach(availableRevision => console.log(availableRevision.date);
};
```

It is possible to get the active revision for any Image360 using the `getActiveRevision()` method, but the `image360Entered` event does also include the revision that was entered.
```ts
 const onImageEntered: Image360EnteredDelegate = (entity: Image360, revision: Image360Revision) => {
      const activeDate = entity.getActiveRevision().date;
      const enteredDate = revision.date;
      alert(`The result is the same: ${activeDate} is equal to ${enteredDate}`);
};
```

To change the revision of an image call `enter360Image(revision)`, and pass in one of the available revisions for that entity.
```ts
const onImageEntered: Image360EnteredDelegate = async (entity: Image360, revision: Image360Revision) => {
  const newRevision = entity.getRevisions().find(availableRevision => availableRevision != revision);
  if(!newRevision){
    //This image only has one revision
    return;
  }
  await entity.changeRevision(newRevision);
  alert(`Revision changed to : ${newRevision.date}`);
};
```

## Image information GUI

To help with navigation and identification of images it is possible to enable a simple GUI that lists relevant metadata for the currently entered image.
Date and time will only be listed if the current revision is dated.

```ts
viewer.show360ImageInformation(true);
```