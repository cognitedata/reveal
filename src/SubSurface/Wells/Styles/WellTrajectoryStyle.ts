...
export class WellTrajectoryStyle extends BaseRenderStyle {
  ...
  public showWellName = new BooleanProperty({ name: "Show Well Name", value: true } );
  
  public showTrajectoryName = new BooleanProperty({ name: "Show Trajectory Name", value: true } );

  public nameColorType = new ColorTypeProperty({ name: "Name Color", value: ColorType.ForeGround });

  public nameFontSize = new NumberProperty({ name: "Name Font Size", value: 50, options: BaseRenderStyle.fontSizeOptions });

  public trajectoryColorType = new ColorTypeProperty({ name: "trajectory Color", value: ColorType.White });
  ...
  public /* override */ clone(): BaseStyle { return cloneDeep<WellTrajectoryStyle>(this); }

  protected /* override */ populateCore(folder: BasePropertyFolder) {
    super.populateCore(folder);

    this.nameColorType.isEnabledDelegate = (): boolean => this.showWellName.value || this.showTrajectoryName.value;
    this.nameFontSize.isEnabledDelegate = (): boolean => this.showWellName.value || this.showTrajectoryName.value;
