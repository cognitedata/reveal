...
describe("Hierarcy", () => {
  ...
  function testHierarcy(): void {
    ...
    {
      let n = 0;
      for (const _ of root.getThisAndDescendants())
        n += 1;
      expect(n).toBe(12);
    }
    {
      let n = 0;
      for (const _ of root.getDescendants())
        n += 1;
      expect(n).toBe(11);
    }
    {
      let n = 0;
      for (const _ of root.getDescendantsByType(PolylinesNode))
        n += 1;
      expect(n).toBe(4);
    }
    {
      let n = 0;
      for (const _ of tree.getChildrenByType(PolylinesNode))
        n += 1;
      expect(n).toBe(4);
    }
