...
import { createPrimitiveInterleavedGeometry, parseInterleavedGeometry } from '../../../../test-utilities/src/primitives/primitiveThreeBuffers';
...
function assertSecondCulledAwayFromBoxAtX10Yneg10Z0(primitives: any[], primitiveType: PrimitiveType) {
  ...
  const filteredGeometry = V9GeometryFilterer.filterGeometry(bufferGeometry,
                                                             collectionType,
                                                             clipBox);
...
describe('V9GeometryFilterer accepts empty clipBox', () => {
  test('no clipbox, returns original', () => {
    ...
    const newGeometry = V9GeometryFilterer.filterGeometry(bufferGeometry,
...
describe('V9GeometryFilterer filters primitives correctly', () => {
  ...
  test('Two boxes: one accepted, one rejected - returns filtered', () => {
    const mat0 = new THREE.Matrix4().makeTranslation(10, -10, 0).multiply(
      new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(0, 1, 2, 'XYZ')));
    const mat1 = new THREE.Matrix4().makeTranslation(-10, 5, 10).multiply(
      new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(0, 1, 2, 'XYZ')));
    const boxes: Box[] = [{
