...
export class Cognite3DViewer {
  ...
  constructor(options: Cognite3DViewerOptions) {
    ...
    this._cameraManager = options.cameraManager ?? new DefaultCameraManager(this.canvas, this._mouseHandler, this.modelIntersectionCallback.bind(this));
  ...
  loadCameraFromModel(model: CogniteModelBase): void {
    ...
    if (config) {
      this._cameraManager.setCameraPosition(config.position);
      this._cameraManager.setCameraTarget(config.target);
    } else {
