...
:::note
The test model used in the documentation site is lacking and the examples shown is somewhat lacking
as a result of this. To best utlize the filtering APIs we recommend providing a rich 3D models 
with a good set of high-quality attributes assosciated with the nodes.
:::
...
Often one would want to match multiple criterias at the same type. Lets say we want to find all
mechanical equipment with purpose 'STU'. We can either execute this as a single query, or as two
seperate queries. 

First, lets look how this looks when combined in a single query:
...
Or, using two separate queries that is combined:
...
The two approaches as their pros and cons. Often it will be faster to combine the 
...
so that the user will want to show 'PIPE'-nodes within the 'STU'-purpose it might 
...
Often it's interesting to invert results from filters to find elements that doesn't match the query. This 
