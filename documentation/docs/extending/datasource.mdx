...
network layer. 
It will require an instance of `BlobOutputMetadata` as input which defines the format, version, and if 
applicable, the remote blob id of the output.
...
The `BlobOutputMetadata` type includes the format, version, and if applicable, the remote blob id of the model. 
