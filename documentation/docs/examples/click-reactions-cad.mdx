...
`Cognite3DViewer.on('clicked', ...)` is used to detect clicks in the 3D model, while 
...
```js runnable
const selectedNodes = new ByTreeIndexNodeSet();
model.addStyledNodeSet(selectedNodes, DefaultNodeAppearance.Highlighted);
...
This can be achieved by determining the an
...
viewer.on('click', event => {
  ...
  if (intersection) {
    // Replace current selection
    model.getAncestorTreeIndices(intersection.treeIndex, 1).then(parentRange => {
