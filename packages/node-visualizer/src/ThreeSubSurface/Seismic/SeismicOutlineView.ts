...
export class SeismicOutlineView extends BaseGroupThreeView {
  ...
  protected get node(): SeismicOutlineNode {
    return super.getNode() as SeismicOutlineNode;
  }
  ...
  public /* override */ mustTouch(): boolean {
    ...
    if (this.fgColor === target.fgColor) return false;
      ...
      for (const j of [0, maxJ])
        for (const k of [0, maxK])
          geometry.vertices.push(
            transformer.to3D(surveyCube.getNodePosition(i, j, k))
          );
    for (const k of [0, maxK]) {
      for (const j of [0, maxJ])
        for (const i of [0, maxI])
