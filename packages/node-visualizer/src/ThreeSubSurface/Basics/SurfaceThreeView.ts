...
export class SurfaceThreeView extends BaseGroupThreeView {
  ...
  protected get node(): SurfaceNode {
    return super.getNode() as SurfaceNode;
  }

  protected get style(): SurfaceRenderStyle {
    return super.getStyle() as SurfaceRenderStyle;
  }
  ...
  protected /* override */ createObject3DCore(): THREE.Object3D | null {
    ...
    if (contours) parent.add(contours);
  ...
  public /* override */ onShowInfo(
    ...
    if (!surface.isNodeInside(cell.i, cell.j)) return;
  ...
  private createSolid(): THREE.Object3D | null {
    const { style } = this;
    if (!style.solidColorType.use) return null;
    ...
    if (style.solidColorType.value === ColorType.ColorMap) {
      ...
      if (!style.solidContour.use)
        ...
        texture = TextureKit.create1DContours(
          ColorMaps.get(node.colorMap),
          grid.boundingBox.z,
          style.increment.value,
          style.solidContour.value
        );
    ...
    } else material.color = ThreeConverter.toThreeColor(color);
  ...
  private createContours(): THREE.Object3D | null {
    const { style } = this;
    if (!style.contourColorType.use) return null;
  ...
  private static setMaterial(
    ...
    if (is2D) material.shininess = 0;
    else
      material.shininess = style.solidShininess.use
        ? 100 * style.solidShininess.value
        : 0;
  }
}
