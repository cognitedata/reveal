...
let mockAssetInstances: AssetInstance[];
let mockAssetMappings: Map<AnnotationId, AssetInstance[]>;
...
mockAssetInstances = [createAssetMock(1), createAssetMock(2)];

mockAssetMappings = new Map([
...
function createPointCloudAnnotationMock(params?: {
  ...
  return {
    ...
    data: {
      assetRef: params?.assetId ? { id: params?.assetId } : undefined,
      instanceRef: params?.instanceId ?? undefined,
      region: []
    },
