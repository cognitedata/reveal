...
import { ClassicDataSourceType, Image360 } from '@cognite/reveal';
import { Image360Details } from './Image360Details';
import { defaultImage360DetailsContextDependencies, Image360DetailsContext } from './Image360Details.context';
import { getMocksByDefaultDependencies } from '#test-utils/vitest-extensions/getMocksByDefaultDependencies';
import { createImage360DmMock } from '#test-utils/fixtures/image360';

describe(Image360Details.name, () => {
  const defaultDependencies = getMocksByDefaultDependencies(defaultImage360DetailsContextDependencies);
  ...
  it('should render Image360HistoricalDetails with enteredEntity', async () => {
    render(<Image360Details />, { wrapper });

    expect(mockImage360.on).toHaveBeenCalledTimes(2);

    const [onEnteredEventType, enteredCallback] = vi.mocked(mockImage360.on).mock.calls[0] as [string, (entity: Image360<ClassicDataSourceType>) => void];
  ...
  it('should clear enteredEntity data on exit', async () => {
    render(<Image360Details />, { wrapper });
  ...
  it('should show exit button by default and trigger exit360Image on click', async () => {
    ...
    const enteredCallback = vi.mocked(mockImage360.on).mock.calls[0][1] as (entity: Image360<ClassicDataSourceType>) => void;
    await act(() => {
      enteredCallback(entity);
    });
  ...
  it('should not show exit button when enableExitButton is false', async () => {
