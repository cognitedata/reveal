...
import { setBoxGeometry, setConeGeometry, setNutGeometry, setQuadGeometry, setTorusGeometry, setTrapeziumGeometry } from "./primitiveGeometries";
import { GltfJson, RevealGeometryCollectionType, Node, GlbHeaderData, GeometryProcessingPayload, TypedArrayConstructor } from "./types";
...
export class RevealGlbParser{
    ...
    public parseSector(data: ArrayBuffer){
        ...
        json.scenes[json.scene].nodes.forEach(nodeId => {
            ...
            const {buffer, type} = this.processNode(node, json, headers, data)!;
    ...
    private processNode(node: Node, json: GltfJson, glbHeaderData: GlbHeaderData, data: ArrayBuffer){
        ...
        if(instancingExtension === undefined) return null;

        const bufferGeometry = instancingExtension 
    ...
    private setInstancedAttributes(payload: GeometryProcessingPayload) {
        ...
        const uniqueBufferViews = [...new Set(Object.values(instancedAttributes).map(accessorId => json.accessors[accessorId].bufferView))];
        
        assert(uniqueBufferViews.length === 1, 'Unexpected number of buffer views');

        const bufferView = json.bufferViews[uniqueBufferViews[0]];

        const offsetToBinChunk = glbHeaderData.gltfHeaderByteSize + glbHeaderData.chunkHeaderByteSize * 2 + glbHeaderData.jsonChunkByteLength;

        const componentTypes = Object.values(instancedAttributes)
            .map(accessorId => json.accessors[accessorId].componentType);

        const typedArrays = [...new Set(componentTypes)]
            .map(componentType => {
                const TypedArray = this.DATA_TYPE_BYTE_SIZES.get(componentType)!;
                const typedBuffer = new TypedArray(data, offsetToBinChunk + (bufferView.byteOffset ?? 0), bufferView.byteLength / TypedArray.BYTES_PER_ELEMENT);
                const interleavedBuffer = new THREE.InstancedInterleavedBuffer(typedBuffer, bufferView.byteStride / TypedArray.BYTES_PER_ELEMENT)
                return {componentType: componentType, interleavedBuffer: interleavedBuffer};
            });
        
        const typedArrayMap: {[key: string]: THREE.InstancedInterleavedBuffer } = Object.assign({}, ...typedArrays.map(p => ({[p.componentType]: p.interleavedBuffer})));
        ...
        Object.keys(instancedAttributes).forEach(attributeName => {
            ...
            const interleavedBufferAttribute = new THREE.InterleavedBufferAttribute(interleavedBuffer, size, (accessor.byteOffset ?? 0) / elementSize);
