...
| Class                          |                                                                                  |
| ------------------------------ | -------------------------------------------------------------------------------- |
| `SimpleNodeCollection`         | A set of nodes defined by a list of tree indices. Can be updated with a new set of indices |
...
| `InvertedNodeCollection`       | Inverts the result of a `NodeCollectionBase` to match the opposite nodes (i.e. a NOT-operator) |
| `AggregateNodeCollection`      | Combines nodes from several `NodeCollectionBase` by applying set operations. Supported operations are [union](https://en.wikipedia.org/wiki/Union_(set_theory)") and [intersection](https://en.wikipedia.org/wiki/Intersection_(set_theory)). |
...
`Cognite3DModel.assignStyleToNodeCollection` and `unassignStyleFromNodeCollection` is used to manage styled set associated with a model:

```js
// import { SimpleNodeCollection, DefaultNodeAppearance, NumericRange } from '@cognite/reveal'

// you can start from binding styles to an empty set and populate it later
const myNodes = new SimpleNodeCollection();
model.assignStyleToNodeCollection(myNodeSet, DefaultNodeAppearance.Highlighted);
...
model.assignStyleToNodeCollection(myNodeSet, DefaultNodeAppearance.Ghosted);

// and when you don't need to style this set anymore, just remove it completely
model.unassignStyleFromNodeCollection(myNodeSet);
```
...
// you can start from binding styles to an empty set and populate it later
const set1 = new SimpleNodeCollection(NumericRange.createFromInterval(0, 100));
model.assignStyleToNodeCollection(set1, { color: [ 255, 0, 0 ] });
const set2 = new SimpleNodeCollection(NumericRange.createFromInterval(50, 150));
model.assignStyleToNodeCollection(set2, { outlineColor: NodeOutlineColor.White });
