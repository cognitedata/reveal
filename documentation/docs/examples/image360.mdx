...
360 images simulate being in the shoes of a photographer and looking around to the left, right, up and down as desired as well as sometimes zooming.
...
The `AddImage360Options` is used for correcting / adding transformations to the set of 360 images and has the following declaration:
```ts
type AddImage360Options = {
  /**
...
It is also possible enter a 360 image programatically by calling the `enter360Image(image360: Image360)` method.
...
The `Image360` instances that are returned have the following form:
