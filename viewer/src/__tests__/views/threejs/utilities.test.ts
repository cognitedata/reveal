...
describe('toThreeVector3', () => {
  test('no out vector, create new vector', () => {
    const result = toThreeVector3(vec3.fromValues(1.0, 2.0, 3.0));
    expect(result).toStrictEqual(new THREE.Vector3(1.0, 2.0, 3.0));
  });

  test('modifies provided out parameter', () => {
    const out = new THREE.Vector3();
    const result = toThreeVector3(vec3.fromValues(1.0, 2.0, 3.0), undefined, out);
  ...
  test('applies model transformation matrix', () => {
    ...
    const result = toThreeVector3(vec3.fromValues(1.0, 1.0, 1.0), transform);
