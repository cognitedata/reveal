...
Annotations are represented by `Image360Annotation` instances. The annotations associated with a given entity revision can be queried with the `annotations` property.
...
Setting individual style can be done directly on an `Image360Annotation` object. Styling fields set this way will override those of the default appearance. To e.g. hide all annotations created by `'user0'` in a revision, you may write
