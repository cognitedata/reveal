...
//If measurement is added, it would have updated line width & color.
```
...
document.onkeydown = function(event) {
  event = event || window.event;
  if (event.keyCode == 32) {
    const measurementRefArray = measurementTool.getAllMeasurements();

    measurementRefArray.forEach(measurementRef => {
        const color = Math.floor(Math.random() * 16777215);
        measurementTool.updateLineColor(measurementRef.measurementId, color);

        //Update line width
        const lineWidth = (Math.random() * 1.0) + 0.2;
        measurementTool.updateLineWidth(measurementRef.measurementId, lineWidth);
        viewer.requestRedraw();
      });
    }
}
...
//On key 'r', remove the first measurement added
document.onkeydown = function(event) {
  event = event || window.event;
  if (event.keyCode == 82) {
