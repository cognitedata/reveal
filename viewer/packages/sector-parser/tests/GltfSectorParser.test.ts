...
describe('Gltf Sector Parser', () => {
  let parsedResult: [RevealGeometryCollectionType, THREE.BufferGeometry][];
  ...
  test('Parsing test.glb should contain 1 box result with 2 instances', () => {
    const boxResult = parsedResult.filter(x => x[0] === RevealGeometryCollectionType.BoxCollection);

    expect(boxResult.length).toBe(1);

    const boxes = boxResult[0][1];
  ...
  test('Parsing test.glb should contain 1 circle result with 10 instances', () => {
    //Assert
    const circleResult = parsedResult.filter(x => x[0] === RevealGeometryCollectionType.CircleCollection);

    expect(circleResult.length).toBe(1);

    const circles = circleResult[0][1];
  ...
  test('Parsing test.glb should contain 1 cone result with 20 instances', () => {
    //Assert
    const coneResult = parsedResult.filter(x => x[0] === RevealGeometryCollectionType.ConeCollection);

    expect(coneResult.length).toBe(1);

    const cones = coneResult[0][1];
  ...
  test('Parsing test.glb should contain 1 eccentricCone result with 2 instances', () => {
    //Assert
    const eccentricConeResult = parsedResult.filter(x => x[0] === RevealGeometryCollectionType.EccentricConeCollection);

    expect(eccentricConeResult.length).toBe(1);

    const eccentricCones = eccentricConeResult[0][1];
  ...
  test('Parsing test.glb should contain 1 ellipsoidSegment result with 8 instances', () => {
    ...
    const ellipsoidSegmentResult = parsedResult.filter(
      x => x[0] === RevealGeometryCollectionType.EllipsoidSegmentCollection
    );

    expect(ellipsoidSegmentResult.length).toBe(1);

    const ellipsoidSegments = ellipsoidSegmentResult[0][1];
  ...
  test('Parsing test.glb should contain 1 generalCylinder result with 10 instances', () => {
    ...
    const generalCylinderResult = parsedResult.filter(
      x => x[0] === RevealGeometryCollectionType.GeneralCylinderCollection
    );

    expect(generalCylinderResult.length).toBe(1);

    const generalCylinders = generalCylinderResult[0][1];
  ...
  test('Parsing test.glb should contain 1 generalRing result with 20 instances', () => {
    //Assert
    const generalRingResult = parsedResult.filter(x => x[0] === RevealGeometryCollectionType.GeneralRingCollection);

    expect(generalRingResult.length).toBe(1);

    const generalRings = generalRingResult[0][1];
  ...
  test('Parsing test.glb should contain 1 quad result with 4 instances', () => {
    //Assert
    const quadResult = parsedResult.filter(x => x[0] === RevealGeometryCollectionType.QuadCollection);

    expect(quadResult.length).toBe(1);

    const quads = quadResult[0][1];
  ...
  test('Parsing test.glb should contain 1 torus result with 4 instances', () => {
    //Assert
    const torusResult = parsedResult.filter(x => x[0] === RevealGeometryCollectionType.TorusSegmentCollection);

    expect(torusResult.length).toBe(1);

    const tori = torusResult[0][1];
  ...
  test('Parsing test.glb should contain 1 trapezium result with 8 instances', () => {
    //Assert
    const trapeziumResult = parsedResult.filter(x => x[0] === RevealGeometryCollectionType.TrapeziumCollection);

    expect(trapeziumResult.length).toBe(1);

    const trapeziums = trapeziumResult[0][1];
