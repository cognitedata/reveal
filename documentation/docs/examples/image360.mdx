...
## Historical images
Some image collections include 360 images with historical revisions. You can use the `360Image` interface to manage which revision of the image is displayed. Use `getRevisions` to obtain a list of all available revisions for an image. This method returns a list of `Image360Revision` objects sorted by date, with the most recent revision at the beginning of the list.
...
const onImageEntered: Image360EnteredDelegate = (entity: Image360, revision: Image360Revision) => {
  entity.getRevisions().forEach(availableRevision => console.log(availableRevision.date);
};
```
