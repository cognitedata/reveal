...
To change width and color of the line, use `MeasurementTool.updateLineOptions(lineOptions: MeasurementLineOptions)`. MeasurementLineOptions expects
  - `lineWidth` : number,
  - `color` : number
...
const measurement = new MeasurementTool(viewer);
measurement.add();
measurement.updateLineOptions(lineOptions);
```
