...
## Historical images
Some image collections include 360 images with historical revisions. You can use the `360Image` interface to manage which revision of the image is displayed. To get a list of all available revisions for a 360 image, use the `getRevisions` method. The method returns a sorted list of `Image360Revision` objects by date, 
...
You can get the currently displayed revision for any `Image360` by using the `getActiveRevision` method. Additionally, you can retrieve the active revision from `image360Entered` as the event includes the revision that was entered.
```ts
 const onImageEntered: Image360EnteredDelegate = (entity: Image360, revision: Image360Revision) => {
...
To change the revision of an `Image360`, you can use the `enter360Image` method. Pass in the `Image360` entity you want to change along with one of the available revisions for that entity.
```ts
const revisions = entity.getRevisions();
...
await viewer.enter360Image(entity, newRevision);
