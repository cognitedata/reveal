...
title: Create custom node collections
...
model.assignStyleToNodeCollection(myNodes, { renderGhosted: false, color: [40, 200, 20] } );
*/
...
model.setDefaultNodeAppearance(DefaultNodeAppearance.Ghosted);
model.assignStyleToNodeCollection(mySet, { renderGhosted: false, color: [40, 200, 20] } );
```
