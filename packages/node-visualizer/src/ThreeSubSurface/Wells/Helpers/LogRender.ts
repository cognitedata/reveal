...
export class LogRender {
  ...
  public createCanvas(zscale: number): Canvas {
    const canvasDy = 128;
    const canvasDx =
      (Math.max(1, zscale) * canvasDy * this.mdRange.delta) /
      this.bandRange.delta;
  ...
  public createBands(
    ...
    const rightBuffers = useRightBand
      ? new TrianglesBuffers(2 * sampleCount, true)
      : null;
    const leftBuffers = useLeftBand
      ? new TrianglesBuffers(2 * sampleCount, true)
      : null;
    if (leftBuffers) leftBuffers.side = THREE.BackSide;
    ...
    for (let md = this.mdRange.min; more; md += mdInc) {
      more = md < this.mdRange.max;
      if (!more) md = this.mdRange.max;

      if (!trajectory.getPositionAtMd(md, position)) continue;

      if (!trajectory.getTangentAtMd(md, tangent)) continue;
      ...
      if (rightBuffers) {
        const startPosition = Vector3.addWithFactor(
          position,
          prependicular,
          this.bandRange.min
        );
        const endPosition = Vector3.addWithFactor(
          position,
          prependicular,
          this.bandRange.max
        );
  ...
  public addFloatLog(
    ...
    if (logValueRange.isEmpty) return;
  ...
  private closePath(
    ...
    if (!canvas.closeFunction(reverse)) return;

    if (fill) canvas.fillPathBySemiTransparentGradient(color, 1, reverse);
    if (lineWidth > 0) canvas.drawPath(color, lineWidth);
  }
  ...
  public addDiscreteLog(canvas: Canvas, log: DiscreteLog | null): void {
    ...
    for (let i = 0; i < log.samples.length; i++) {
      ...
      const color = sample.isEmpty
        ? Colors.grey
        : Colors.getNextColor(sample.value);
      const mdFraction = this.mdRange.getFraction(sample.md);
      if (i > 0) canvas.fillRect(prevMdFraction, mdFraction, prevColor, 0.75);
