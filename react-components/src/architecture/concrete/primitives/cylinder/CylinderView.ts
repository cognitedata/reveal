...
import { type CylinderRenderStyle } from './CylinderRenderStyle';
...
export class CylinderView extends GroupThreeView<CylinderDomainObject> {
  ...
  protected override get style(): CylinderRenderStyle {
    return super.style as CylinderRenderStyle;
  }
  ...
  private createEdgeCircle(matrix: Matrix4, material: Material, face: BoxFace): Mesh | undefined {
    ...
    if (!CylinderDomainObject.isValidSize(adjacentSize1)) {
      return undefined;
    }
    const adjacentSize2 = domainObject.size.getComponent(face.tangentIndex2);
    if (!CylinderDomainObject.isValidSize(adjacentSize2)) {
      return undefined;
    }
  ...
  private addLabels(matrix: Matrix4): void {
    ...
    for (let index = 0; index < 3; index++) {
      ...
      if (!CylinderDomainObject.isValidSize(size)) {
...
function rotate(mesh: Mesh, face: BoxFace, domainObject: CylinderDomainObject): void {
