...
function is_demo_app() {
  git remote get-url origin | grep 'infield2.git' >/dev/null 2>&1
  return $?
}
...
# bazel run :start --max-old-space-size=4096 // Error: Option max-old-space-size not recognized
bazel run :start
