...
async function main() {
  ...
  try {
    ...
    shell(`mkdir -p ${buildDir}/static/static`);
    shell(
      `[ ! -f ${buildDir}/static/js ] || mv ${buildDir}/static/js ${buildDir}/static/static`
    );
    shell(
      `[ ! -f ${buildDir}/static/media ] || mv ${buildDir}/static/media ${buildDir}/static/static`
    );
    shell(
      `[ ! -f ${buildDir}/index.js ] || mv ${buildDir}/index.js ${buildDir}/static`
    );
    shell(
      `[ ! -f ${buildDir}/index.js.map ] || mv ${buildDir}/index.js.map ${buildDir}/static`
    );

    // Copy in files from fusion.cognite.com
    shell(
      `[ ! -f ${tempFolderPath}/index.html ] || mv ${tempFolderPath}/index.html ${buildDir}/`
    );
    shell(
      `[ ! -f ${tempFolderPath}/* ] || mv ${tempFolderPath}/* ${buildDir}/`
    );
