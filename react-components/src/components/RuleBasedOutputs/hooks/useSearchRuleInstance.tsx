...
export const useSearchRuleInstance = (): ((externalId: string) => Promise<Array<NodeItem<RuleOutputSet>>>) => {
  const fdmSdk = useFdmSdk();

  const searchRuleById = useCallback(async (ruleId: string): Promise<Array<NodeItem<RuleOutputSet>>> => {
    ...
    const result = await fdmSdk.searchInstances<RuleOutputSet>(RULE_BASED_OUTPUTS_VIEW, '', 'node', 1, filter);
