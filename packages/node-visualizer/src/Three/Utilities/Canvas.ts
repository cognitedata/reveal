...
export class Canvas {
  ...
  public closeFunction(reverse: boolean): boolean {
    if (Number.isNaN(this.firstX)) return false;
  ...
  public addFunctionValue(
    ...
    if (colorMap) {
      ...
      this.gradient.addColorStop(
        xFraction,
        Canvas.getColor(colorMap.getColor(yFraction))
      );
    }
    ...
    if (Number.isNaN(this.firstX)) {
      this.beginPath();
      this.firstX = x;
      if (this.fillFunction) this.context.moveTo(x, reverse ? this.dy : 0);
    }
  ...
  public drawText(
    ...
    this.context.translate(
      x * this.dx - borderSize,
      outerMost ? this.dy - borderSize : borderSize
    );

    this.context.rotate(-Math.PI / 2);
    if (!rightBand) this.context.scale(-1, 1);

    if (rightBand === outerMost) this.context.textAlign = 'left';
    else this.context.textAlign = 'right';
  ...
  public fillPathBySemiTransparentGradient(
    color: Color,
  ...
  public static getColor(color: Color | null) {
    return color ? color.string() : 'black';
  }

  private static getFont(fontSize: number): string {
    return `${fontSize}px ${Appearance.viewerFontType}`;
  }

  public static getNormalFont(fontSize: number): string {
    return `Normal ${Canvas.getFont(fontSize)}`;
  }

  public static getBoldFont(fontSize: number): string {
    return `Bold ${Canvas.getFont(fontSize)}`;
  }

  public static getBolderFont(fontSize: number): string {
    return `Bolder ${Canvas.getFont(fontSize)}`;
  }

  public static measureTextHeight(
    context: CanvasRenderingContext2D,
    text: string,
    maxWidth: number,
    lineHeight: number
  ): number {
    return Canvas.fillOrTextHeightText(
      context,
      text,
      -1,
      -1,
  ...
  private static fillOrTextHeightText(
    context: CanvasRenderingContext2D,
    text: string,
    x: number,
    y: number,
