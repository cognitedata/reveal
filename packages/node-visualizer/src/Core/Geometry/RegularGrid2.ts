...
export class RegularGrid2 extends Grid2 {
  ...
  public /* override */ toString(): string {
    return `nodeSize: (${this.nodeSize}) origin: (${this.origin.x}, ${this.origin.y}) inc: (${this.inc.x}, ${this.inc.y})`;
  }
  ...
  public expandBoundingBox(boundingBox: Range3): void {
    ...
    for (let j = this.nodeSize.j - 1; j >= 0; j--) {
      for (let i = this.nodeSize.i - 1; i >= 0; i--) {
        if (this.getNodePosition(i, j, position)) boundingBox.add(position);
      }
    }
  }
  ...
  public getNodePosition(
    ...
    if (this._hasRotationAngle) {
      ...
      resultPosition.x =
        dx * this._cosRotationAngle - dy * this._sinRotationAngle;
      resultPosition.y =
        dx * this._sinRotationAngle + dy * this._cosRotationAngle;
    } else {
  ...
  public getNormal(
    ...
    if (normalize && !resultNormal.normalize()) resultNormal.set(0, 0, 1);
    return resultNormal;
  }
