...
:::note
Recommended: `CameraControlsOptions.changeCameraTargetOnClick` should be set to `false` to avoid click event registering for camera when `MeasurementTool` is active. Also avoid any custom click-handler that might interfere with the measurement tool.
:::
...
const measurementTool = new MeasurementTool(viewer);
let measurementIterator;
measurementTool.on('ended',
  () => {
    //Changes measurement line color & width to random values.
    setInterval(() => {
      const measurement = measurementIterator.next().value[1];
...
//get all measurements
setTimeout(() => {
  measurementIterator = measurementTool.getAllMeasurements().entries();
  measurementTool.exitMeasurementMode();
}, 8000);
```
