...
Currently, class provided by Reveal is `AnnotationIdPointCloudObjectCollection` for CDF's Annotations, which is initialized with a list of annotation IDs corresponding to stylable objects.

Or use class `PointCloudDMVolumeCollection` for CDF's CognitePointCloudVolume, which is initialized with list of point cloud volume references corresponding to stylable objects.
...
model.stylableObjects.forEach(objectMetadata => {
  if (isDMPointCloudDataType(objectMetadata))
