...
export function transformDeleteRequest(
  ...
  if (storeKey && req.body && req.body.items.length) {
    ...
    // internally, we are using id (CDF does this too) so we should use id for deleting instead
...
export function transformPostCreateRequest(
  ...
  const autogeneratedId = uuid.generate();
  if (!payload.externalId) {
    payload.externalId = autogeneratedId;
  }

  if (!payload.id) {
    payload.id = autogeneratedId;
  }
