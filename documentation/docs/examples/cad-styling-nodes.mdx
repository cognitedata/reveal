...
`ByIdsNodeCollection` contains a list of nodes identified by their CDF IDs.
...
If Node IDs are known, `ByIdsNodeCollection` can be used to style the nodes (and the children):

```js runnable
// import { ByIdsNodeCollection, DefaultNodeAppearance } from '@cognite/reveal';

const nodes = new ByIdsNodeCollection(sdk, model);
const nodeIds = [...new Array(100).keys()].map(x => 100*x);
