...
:::caution Remove the previous overlay before loading a new one, and guard against overlapping calls
If your application changes the overlaid node set over time (e.g. a different selection per navigation step), you **must** remove the existing overlay model and its styled collections before loading the next one. Skipping this causes overlay models and styled node collections to accumulate on every switch, which severely degrades performance — each accumulated overlay keeps loading and rendering geometry, and each accumulated styled collection keeps being evaluated.

This alone is not sufficient if `loadOverlayView` can be called again before a previous call finishes `await`ing (e.g. rapid selection changes): since network requests can resolve out of call order, an older, slower call could finish last and overwrite a newer call's result. Use a generation counter to discard the result of any call that has since been superseded, as shown below.
:::
...
async function loadOverlayView(nodeIds) {
  ...
  const newOverlayModel = await viewer.addCadModel({
    modelId,
    revisionId,
    outputFormat: File3dFormat.GltfPrioritizedNodes
  });
  ...
  if (generation !== overlayLoadGeneration) {
    // A newer loadOverlayView() call started while we were awaiting — discard this model so an
    // older, slower-resolving request can't overwrite a newer one's state.
    viewer.removeModel(newOverlayModel);
    return;
  }
...
:::caution Guard against overlapping calls with a generation counter
Locking a new set of nodes does not automatically release a previously locked set. If your application locks a different set of nodes over time (e.g. as the user changes selection), you must release the old lock before applying the new one — otherwise locked tree indices and styled collections accumulate across every switch, and locked sectors are never evicted (see [Limitations](#limitations)), so the render budget consumed by locking keeps growing.

Simply placing the cleanup right before `assignStyledNodeCollection`/`lockTreeIndices` is not enough on its own: since `executeFilter` is asynchronous, two overlapping calls can resolve **out of call order** (e.g. a later call's network request finishes faster than an earlier, slower one). Whichever call happens to resolve *last* would then win, even if it was actually the *older*, now-stale request — leaving the UI showing outdated locked nodes instead of the user's latest selection.

The robust fix is a generation counter: increment it on every call, capture the value before the `await`, and after the `await` discard the result if a newer call has since started. This guarantees only the most recently *initiated* request's result is ever applied, regardless of resolution order.
:::
...
async function lockNodes(nodeIds) {
  ...
  if (generation !== lockGeneration) {
    // A newer lockNodes() call started while we were awaiting — discard this result so an
    // older, slower-resolving request can't overwrite a newer one's state.
    return;
  }
  ...
  // Release any previously locked set right before applying the new one. A stale
  // (superseded) call was already discarded above, so this can never accumulate.
...
function unlockNodes() {
  // Invalidate any in-flight lockNodes() call so it can't re-apply after this clears state.
  ++lockGeneration;
  mainModel.unlockAllTreeIndices();
  mainModel.removeAllStyledNodeCollections();
}
