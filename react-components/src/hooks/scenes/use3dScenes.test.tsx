...
describe(use3dScenes.name, () => {
  ...
  test('should handle pagination', async () => {
    ...
    expect(allScenes).toHaveLength(15);
    expect(mockQueryNodesAndEdges).toHaveBeenCalledTimes(2);
  });

  test('should handle pagination for related data when hitting limits', async () => {
    // Create a large number of models to trigger pagination
    const createModels = (count: number, sceneExternalId: string) =>
      Array.from({ length: count }, (_, i) => ({
        startNode: { externalId: sceneExternalId, space: 'test-space' },
        endNode: { externalId: `cog_3d_model_${i}` },
        properties: {
          scene: {
    ...
    // First query returns 1 scene with 1000 models (hitting the limit)
    mockQueryNodesAndEdges
      .mockResolvedValueOnce({
        items: {
          scenes: [createSceneNode('scene-with-many-models', 'test-space')],
          sceneModels: createModels(1000, 'scene-with-many-models'), // Hit the limit
          scene360Collections: [],
          sceneGroundPlanes: [],
          sceneGroundPlaneEdges: [],
          sceneSkybox: []
      ...
      // Additional paginated query for models
      .mockResolvedValueOnce({
        items: {
          sceneModels: createModels(500, 'scene-with-many-models') // Additional models
        },
        nextCursor: undefined
      });

    const result = await executeQueryFn();
    const scene = result['test-space']['scene-with-many-models'];

    expect(scene.modelOptions).toHaveLength(1500); // Total models from both queries
    expect(mockQueryNodesAndEdges).toHaveBeenCalledTimes(2); // Initial query + paginated models query
  });
