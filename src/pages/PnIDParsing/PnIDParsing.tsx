...
export default function PnidParsing() {
  ...
  const onLinkAssetsClick = (e: ClickParam, file: FileInfo) => {
    switch (e.key) {
      case 'link':
        if (canEditFiles) {
          dispatch(
            linkFileWithAssetsFromAnnotations.action({ fileId: file.id })
          );
        } else {
          setRenderFeedback(true);
        }
          ...
          <>
            ...
            >
              <Button
                ...
                disabled={
                  !(file && file.annotations && file.annotations.length > 0)
                }
