...
export JEST_JUNIT_OUTPUT_DIR="$(abspath "$TEST_UNDECLARED_OUTPUTS_DIR/test.outputs/$SERVICE_NAME")"

# Directory to save coverage for jest
exec "${@:2}" --coverageDirectory="$(abspath "$TEST_UNDECLARED_OUTPUTS_DIR/test.outputs/$SERVICE_NAME/coverage")"
