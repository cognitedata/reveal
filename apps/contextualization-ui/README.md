...
5. Find `@cognite/cdf-contextualization-ui` module name and click on the row including it
6. Override using `https://localhost:3020/index.js` (port might differ depending on your local settings)
...
1. Start your app by passing the configuration parameter
   ```
   NODE_ENV=mock nx serve contextualization-ui --configuration mock
   ```
2. Start the mock server `nx serve mock-server`
3. Open browser on `https://localhost:3020/test-app`

## Troubleshooting

### I get "Refused to load the script `https://localhost:3020/index.js"` error when I apply a subapp override

1.  Open `https://localhost:3020/index.js` in a new tab.
