...
export class NodeUtils {
  ...
  public static getTreeRoot(): BaseNode | null {
    if (BaseRootNode.active) return BaseRootNode.active;
    return null;
  }
  ...
  public static createElementTree(
    ...
    if (property instanceof GroupProperty) {
      ...
      for (const childProperty of property.children) {
        ...
        if (childElement) parentElement.subValues?.push(childElement);
      }
    ...
    if (property instanceof ExpanderProperty) {
      ...
      for (const childProperty of property.children) {
        const childElement = NodeUtils.createElementTree(
          childProperty
        ) as ISettingsElement;
        if (childElement) parentElement.elements.push(childElement);
      }
      return parentElement;
    }
    // ***** ValueProperty
  ...
  private static createExpanderElement(
    property: ExpanderProperty
  ): ISettingsSection {
  ...
  private static createControlElement(
    property: BaseProperty
  ): ISettingsElement | null {
    if (!(property instanceof ValueProperty)) return null;

    if (!property.isEnabled) return null;
    ...
    const element: ISettingsElement = {
      ...
      options: NodeUtils.createSelectOptions(
        property.getExpandedOptions(),
        property.getOptionIcon
      ),
  ...
  private static getControlType(property: ValueProperty<any>): string | null {
    ...
    if (property instanceof BooleanProperty) return ElementTypes.Boolean;
    if (property instanceof SliderProperty) return ElementTypes.Slider;
    if (property instanceof ColorProperty) return ElementTypes.Color;
    if (property instanceof ColorMapProperty) return ElementTypes.ColorMap;
    if (property instanceof ColorTypeProperty) return ElementTypes.ColorType;

    // All others with options
    if (property.hasOptions) return ElementTypes.Select;

    // All without options
    if (property instanceof StringProperty) return ElementTypes.String;
    if (property instanceof NumberProperty) return ElementTypes.Number;
