...
| `AggregateNodeCollection`      | Combines nodes from several node colletions by applying set operations. Supported operations are [union](https://en.wikipedia.org/wiki/Union_(set_theory)") and [intersection](https://en.wikipedia.org/wiki/Intersection_(set_theory)). |
...
model.assignStyleToNodeCollection(myNodes, DefaultNodeAppearance.Highlighted);
...
model.assignStyleToNodeCollection(myNodes, DefaultNodeAppearance.Ghosted);

// and when you don't need to style this set anymore, just remove it completely
model.unassignStyleFromNodeCollection(myNodes);
```
