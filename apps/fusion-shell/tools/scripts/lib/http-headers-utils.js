function appendHeader(cspHeaderGroups, url) {
...
function generateCSPHeader(appsManifest, environment = 'staging') {
  const cspHeaderGroups = {
    'default-src': [
  ...
  appsManifest.apps
    ...
    .forEach((app) => appendHeader(cspHeaderGroups, app.hosting[environment]));

  if (environment !== 'production') {
    appendHeader(cspHeaderGroups, 'localhost:* *.localhost:*');
    // appendHeader(cspHeaderGroups, 'https://localhost:*');
  }
