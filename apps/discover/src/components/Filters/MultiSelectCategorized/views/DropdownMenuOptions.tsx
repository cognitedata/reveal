...
export const DropdownMenuOptions: React.FC<DropdownViewOption> = ({
  ...
  return (
    <OptionsCategoryWrapper data-testid="option" onMouseEnter={onHoverCategory}>
      <Menu.Submenu
        visible={visibleCategory === category}
        hideOnClick={undefined}
        disabled={options?.every(
          (item) => item.label === EMPTY_SUBMENU_OPTIONS
        )}
        content={
          <DropdownContentWrapper onMouseEnter={onHoverCategory}>
            <DropdownContent>
              {(options || []).map((option) => {
