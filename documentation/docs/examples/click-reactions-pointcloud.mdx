...
In order to retrieve information about a clicked object, users may get the `annotationId` or the `assetId` that is assigned to each point.
The `annotationId` and `assetId` are available in the `intersection` object returned from `viewer.getIntersectionFromPixel`.
An annotation id of 0 means that the clicked point is not assigned to any object. `assetId` will only be defined if the annotation associated with the point is linked to an asset in CDF.
...
viewer.on('click', async event => {
  ...
  if (intersection) {
     alert(`Clicked object with annotationId ${intersection.annotationId} and assetId ${intersection.assetId}`);
  };
});
```
