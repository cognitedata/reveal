...
export function getComponentByteSize(type: AttributeComponentType) {
  switch (type) {
    case 'float':
      return 4;
    case 'byte':
...
export function getShouldNormalize(type: AttributeComponentType) {
  switch (type) {
    case 'byte':
      return true;
    default:
...
const primitiveAttributeNameMap: Map<PrimitiveType, string[]> = new Map([
  [PrimitiveType.Ellipsoid, ['_treeIndex', '_color', '_center', '_normal', '_horizontalRadius', '_verticalRadius', '_height']],
...
export function getTotalAttributeSize(name: PrimitiveType) {
...
export function createAttributeDescriptionsForPrimitive(name: PrimitiveType) {
