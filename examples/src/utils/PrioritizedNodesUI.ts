...
export class PrioritizedNodesUI {
    ...
    const actions = {
      lock: async () => {
        ...
        try {
          ...
          logDetail(
            'Resolved %d node IDs → %d tree indices: [%s]',
            nodeIds.length,
            treeIndices.length,
            treeIndices.join(', ')
          );
  ...
  private async loadOverlayView(nodeIds: number[]): Promise<void> {
    ...
    logDetail(
      'Step 1: Ghost all %d nodes in original model (standard-detail) → semi-transparent context',
      this._mainModel.nodeCount
    );
    this._mainModel.setDefaultNodeAppearance(DefaultNodeAppearance.Ghosted);

    logDetail(
      'Step 2: Hide %d prioritized nodes in original model (standard-detail) → removes their low-detail geometry from view',
      nodeIds.length
    );
    ...
    logDetail(
      '         %d node IDs → %d tree indices resolved on original model',
      nodeIds.length,
      mainTreeIndices.count
    );
    ...
    logDetail(
      'Step 4: Show %d prioritized nodes in overlay model (high-detail) → replaces hidden standard-detail with high-detail geometry',
      nodeIds.length
    );
    ...
    logDetail(
      '         %d node IDs → %d tree indices resolved on overlay model',
      nodeIds.length,
      overlayTreeIndices.count
    );
