...
with status S5". Combinations are also possible by using `AggregateNodeCollection`.
...
to match. To emulate `OR`, use `AggregateNodeCollection` with a `union`-operator.
...
model.assignStyleToNodeCollection(allPipes, { renderGhosted: false, color: [40, 200, 20] } );
```
...
model.setDefaultNodeAppearance(DefaultNodeAppearance.Ghosted);
model.assignStyleToNodeCollection(nodesFromFile, { renderGhosted: false, outlineColor: NodeOutlineColor.Orange } );
```
...
When doing this as a single query, no `AggregateNodeCollection` is necessary and the code is quite compact:
...
model.setDefaultNodeAppearance(DefaultNodeAppearance.Ghosted);
model.assignStyleToNodeCollection(stuMechNodes, { renderGhosted: false, color: [40, 200, 20] } );
```

Combining two separate queries requires the use of `AggregateNodeCollection`:

```js runnable
// import { PropertyFilterNodeCollection, AggregateNodeCollection, DefaultNodeAppearance } from '@cognite/reveal';
...
const combined = new AggregateNodeCollection('intersection');
combined.add(mechNodes);
combined.add(stuNodes);

model.setDefaultNodeAppearance(DefaultNodeAppearance.Ghosted);
model.assignStyleToNodeCollection(combined, { renderGhosted: false, color: [40, 200, 20] } );
```
...
model.setDefaultNodeAppearance(DefaultNodeAppearance.Ghosted);
model.assignStyleToNodeCollection(notLivingQuarters, { renderGhosted: false } );
```
