...
`Cognite3DModel.addStyledNodeSet`, `changeStyledNodeSetAppearance` and `removeStyledNodeSet` is used to manage styled set associated with a model:

```js
// import { ByTreeIndexNodeSet, DefaultNodeAppearance, NumericRange } from '@cognite/reveal'

// you can start from binding styles to an empty set and populate it later
const myNodeSet = new ByTreeIndexNodeSet();
model.addStyledNodeSet(myNodeSet, DefaultNodeAppearance.Highlighted);
...
myNodeSet.updateSet(new IndexSet(new NumericRange(nodeTreeIndex, nodeSubtreeSize)));

// if you want to change appearance of your existing set you can do it like this
model.changeStyledNodeSetAppearance(myNodeSet, DefaultNodeAppearance.Ghosted);

// and when you don't need to style this set anymore, just remove it completely
model.removeStyledNodeSet(myNodeSet);
```
...
// you can start from binding styles to an empty set and populate it later
const set1 = new ByTreeIndexNodeSet(new NumericRange(0, 100));
model.addStyledNodeSet(set1, { color: [ 255, 0, 0 ] });
const set2 = new ByTreeIndexNodeSet(new NumericRange(50, 100));
model.addStyledNodeSet(set2, { outlineColor: NodeOutlineColor.White });

// [0..50] will be colored red
// [50..100] will be colored red and have a white outline
// [100..150] will have the default color and a white outline
```
