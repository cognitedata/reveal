...
// Branchless digit extraction - replaces pow() with step-based lookup for positions 0-2
...
// Branchless digit count using step functions
...
void main() {
  ...
  // Branchless texture sampling - sample both textures and blend
  ...
  // Branchless hover tint for clusters
  // Calculate tinted version always, then blend based on conditions
  ...
  // Apply hover effect only for hovered clusters (branchless)
  ...
  if (shouldRenderNumber > 0.5) {
    ...
    if (inNumberArea > 0.5) {
      ...
      // Branchless character value calculation for "99+" case
      ...
      // Branchless composite - blend based on alpha threshold
