...
export class TrajectoryBufferGeometry extends THREE.BufferGeometry {
  ...
  public constructor(samples: RenderSample[], radialSegments = 16) {
    ...
    const difference = Vector3.substract(
      samples[0].point,
    ...
    this.setAttribute(
      'color',
      new THREE.Uint8BufferAttribute(this.colors, 3, true)
    );
  }
  ...
  private generateBufferData(samples: RenderSample[]) {
    ...
    for (let i = 0; i <= maxIndex; i++) {
      ...
      if (currSample.isEmpty && prevSample.isEmpty) continue;
      ...
      // Continue sampling
      else this.addSample(currSample, tangent, true);
  ...
  private addBaseCap(
    sample: RenderSample,
    tangent: Vector3,
    prevSample: RenderSample | null = null
  ) {
    const normalDirection = -1;
    if (!prevSample) this.addSample(sample, tangent, false, normalDirection);
    else
      this.addSegment(
        sample.point,
        prevSample.radius,
  ...
  private addRadiusShift(
    prevSample: RenderSample,
    sample: RenderSample,
    tangent: Vector3
  ) {
  ...
  private addSegment(
    ...
    if (normalDirection) tangent.normalize();
    ...
    for (let j = 0; j <= this.radialSegments; j++) {
      ...
      if (normalDirection === 1)
        ...
        this.normals.push(-tangent.x, -tangent.y, -tangent.z);
      else this.normals.push(this.normal.x, this.normal.y, this.normal.z);
    ...
    if (generateIndices) this.generateIndices();
  }
  ...
  private generateUVs(samples: RenderSample[]): void {
    ...
    for (let i = 0; i < samples.length; i++) {
      ...
      for (let j = 0; j <= this.radialSegments; j++) this.uvs.push(uv, 1);
    }
  }
}
