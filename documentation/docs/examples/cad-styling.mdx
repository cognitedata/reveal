...
```js runnable
const assetMappedNodes = new ByAssetNodeSet(sdk, model);
// When no asset ID is provided, all nodes mapped to assets are included
assetMappedNodes.executeFilter({});
model.setDefaultNodeAppearance(DefaultNodeAppearance.Ghosted);
model.addStyledNodeSet(assetMappedNodes, {renderGhosted: false, outlineColor: NodeOutlineColor.Cyan } );
```
...
const assetMappedNodes = new ByAssetNodeSet(sdk, model);
// When no asset ID is provided, all nodes mapped to assets are included
assetMappedNodes.executeFilter({ assetId: 4222791488928479 });
model.setDefaultNodeAppearance(DefaultNodeAppearance.Ghosted);
model.addStyledNodeSet(assetMappedNodes, {renderGhosted: false, outlineColor: NodeOutlineColor.Cyan } );
```

#### Style assets without an area

To support proximity lookups

Furthermore, a bounding box can be provided to only load assets within a given area:
```js runnable
```


# Custom NodeSet implementation
