 ...
 */
export const getScaleBlocksByHeight = (max: number, scaleLength: number) => {
  /**
  ...
  const nticks = Math.floor((scaleLength - SCALE_PADDING) / SCALE_BLOCK_HEIGHT);

  return getScaleBlocksByCount({ max, nticks });
};
