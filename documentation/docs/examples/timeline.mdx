...
const playLength = 25000;

timeline.play(new Date('2021-10-05'), new Date('2021-10-09'), playLength);

let refDate = 0;

timeline.subscribe('dateChanged', (data) => {
  if(refDate === 0) {
    refDate = data.date.getTime();
  }
  const width = ((data.date.getTime() - refDate) / playLength) * 100;
  const widthPercentage = width.toString() + '%';
