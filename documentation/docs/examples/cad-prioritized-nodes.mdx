...
:::caution Unlock the previous set right before locking a new one — not before resolving node IDs
Locking a new set of nodes does not automatically release a previously locked set. If your application locks a different set of nodes over time (e.g. as the user changes selection), call `mainModel.unlockAllTreeIndices()` and `mainModel.removeAllStyledNodeCollections()` before locking the new set — otherwise locked tree indices and styled collections accumulate across every switch, and locked sectors are never evicted (see [Limitations](#limitations)), so the render budget consumed by locking keeps growing.

Place this cleanup **after** resolving `nodeIds` to tree indices (i.e. after `await nodeCollection.executeFilter(...)`), immediately before `assignStyledNodeCollection`/`lockTreeIndices`, not before it. If `lockNodes` can be called again before a previous call resolves (e.g. rapid selection changes), cleaning up before the `await` lets both calls clear the old state first and then each append their own result once resolved — reproducing the exact accumulation this is meant to prevent. Placing cleanup and locking together after the `await`, with no `await` in between, ensures whichever call resolves last atomically clears anything applied by an earlier call before applying its own.
:::
...
async function lockNodes(nodeIds) {
  ...
  // `await` above) so that if `lockNodes` is called again before this resolves, whichever
  // call resolves last always wins instead of both accumulating.
