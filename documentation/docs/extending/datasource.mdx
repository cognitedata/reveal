...
[`CdfModelIdentifier`](../API%20Reference#class-cdfmodelidentifier):
```js
...
| Function/Field              | Required?                                  | Description                                                      |
| --------------------------- | ------------------------------------------ | ----------------------------------------------------------------- |
| `mapTreeIndicesToNodeIds()` | No, but recommended to have a 1:1 mapping  | Maps from Reveal tree indices to CDF node IDs used to identify nodes in CDF |
...
| `Cognite3DModel` \ `NodesApiClient` | <div >`mapTreeIndicesToNodeIds()`</div> | `mapNodeIdsToTreeIndices()` | `determineTreeIndexAndSubtreeSizesByNodeIds()` | `determineNodeAncestorsByNodeId()` | `getBoundingBoxByNodeId()` |
| ----------------------------------- | --------------------------------------- | --------------------------- | ---------------------------------------------- | ---------------------------------- | -------------------------- |
