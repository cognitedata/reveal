...
// DragControls will stop navigation from working because it stops events
// Ensure it's only enabled when hovering over the dragger
const raycaster = new THREE.Raycaster();
viewer.domElement.addEventListener('mousemove', ev => {
  const normalizedCoords = new THREE.Vector2(
    ev.offsetX / viewer.domElement.clientWidth * 2 - 1,
    ev.offsetY / viewer.domElement.clientHeight * -2 + 1
  );

  raycaster.setFromCamera(normalizedCoords, viewer.getCamera());
  const intersections = raycaster.intersectObject(draggerMesh);
