...
describe('createShapeBoundingBox', () => {
  test('bounding box for unit box is unit box', () => {
    const box = createShapeBoundingBox({ shapeType: ShapeType.Box, invMatrix: { data: new THREE.Matrix4().identity().toArray() } } as Box);
  ...
  test('bounding box for unit cylinder is unit box', () => {
    const box = createShapeBoundingBox({ shapeType: ShapeType.Cylinder, centerA: [0.0, -0.5, 0.0], centerB: [0.0, 0.5, 0.0], radius: 0.5 } as Cylinder);
