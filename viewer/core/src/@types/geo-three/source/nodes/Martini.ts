...
class Tile 
{
	...
	public getMesh(maxError: number = 0, withSkirts: boolean = false): {vertices: any, triangles: any, numVerticesWithoutSkirts: number} 
	{
		...
		function constructSkirt(skirt: number[], skirtIndex: number): void {
		...
		if (withSkirts) 
		{
			...
			constructSkirt(leftSkirtIndices, skirtIndex);
			constructSkirt(rightSkirtIndices, skirtIndex);
			constructSkirt(bottomSkirtIndices, skirtIndex);
			constructSkirt(topSkirtIndices, skirtIndex);
		}
