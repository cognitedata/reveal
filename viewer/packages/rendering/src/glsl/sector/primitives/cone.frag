...
void main()
{
  ...
  if (intersectionPoint.z <= 0.0 ||
      ...
      // construction of the cone normal has an inconsistent sign across the
      // axis-plane seam; the first surface a ray hits has its outward normal
      // facing the camera, so orient against the actual ray direction. This is
      // more correct than a constant +Z (which only approximates the view ray
      // and leaves the two halves inconsistent when the normal is used in
      // world/model space, e.g. for lighting).
