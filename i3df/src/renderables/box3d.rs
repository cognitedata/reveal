...
impl ToRenderables for crate::Box3D {
    fn to_renderables(&self, collections: &mut PrimitiveCollections) {
        ...
        let second_rotation = Rotation3::rotation_between(&Vector3::z_axis(), &self.normal.into()).unwrap();
        let scale_matrix = Matrix4::new_nonuniform_scaling(&self.delta());

        let instance_matrix = Matrix4::from(translation_matrix) * Matrix4::from(second_rotation) * Matrix4::from(first_rotation) * scale_matrix;
