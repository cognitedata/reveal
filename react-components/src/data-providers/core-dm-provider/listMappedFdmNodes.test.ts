...
import { listMappedFdmNodes, queryCadAssets, queryPointCloudsAssets, queryImage360Assets, listAllMappedFdmNodes } from './listMappedFdmNodes';
import { Mock, It } from 'moq.ts';
import { type DmsUniqueIdentifier, type FdmSDK } from '../FdmSDK';
import { COGNITE_CAD_NODE_SOURCE } from './dataModels';
import { cogniteAssetSourceFixture, simpleSourcesFixtures } from '../../../tests/tests-utilities/fixtures/dm/sources';
import { cadNodesFixtures, object3dIdentifierFixture } from '../../../tests/tests-utilities/fixtures/dm/nodeItems';
...
describe(listMappedFdmNodes.name, async () => {
  ...
  it('lists nothing when network request returns empty result', async () => {
    ...
    const resultData = Promise.resolve({ items: { cad_nodes: [],  cad_assets: [], pointcloud_assets: [], image360_assets: [] } });

    const fdmSdkMock = new Mock<FdmSDK>()
      .setup(async (p) => await p.queryAllNodesAndEdges(
        It.IsAny(), It.IsAny())
      )
      .returns(resultData)
      .object();

    const result = await listMappedFdmNodes(revisionRefs, simpleSourcesFixtures, undefined, 1000, fdmSdkMock);
