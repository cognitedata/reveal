...
export abstract class BasePropertyFolder extends BaseProperty {
  ...
  public get children(): BaseProperty[] {
  ...
  public /* override */ clearDelegates(): void {
    for (const child of this.children) child.clearDelegates();
  }
  ...
  public /* virtual */ createGroup(_name: string): BasePropertyFolder {
    throw Error('Cannot make a group here');
  }

  public /* virtual */ createExpander(_name: string): BasePropertyFolder {
    throw Error('Cannot make a folder here');
  }
  ...
  public addBoolean(params: IPropertyParams<boolean>): void {
    this.addChild(new BooleanProperty(params));
  }

  public addNumber(params: IPropertyParams<number>): void {
    this.addChild(new NumberProperty(params));
  }

  public addSlider(params: IPropertyParams<number>) {
    this.addChild(new SliderProperty(params));
  }

  public addString(params: IPropertyParams<string>): void {
    this.addChild(new StringProperty(params));
  }

  public addColor(params: IPropertyParams<Color>): void {
    this.addChild(new ColorProperty(params));
  }

  public addColorType(params: IPropertyParams<ColorType>): void {
    this.addChild(new ColorTypeProperty(params));
  }
  ...
  public addReadOnlyString(name: string, value: string): void {
    this.addChild(new StringProperty({ name, value, readonly: true }));
  }

  public addReadOnlyInteger(name: string, value: number): void {
    this.addReadOnlyString(name, value.toString());
  }

  public addReadOnlyNumber(
    name: string,
    value: number,
    fractionDigits = FractionDigitsDefault
  ): void {
    this.addReadOnlyString(name, value.toFixed(fractionDigits));
  }

  public addReadOnlyAngle(name: string, value: number): void {
    this.addReadOnlyString(name, `${Ma.toDeg(value).toFixed(2)} [degrees]`);
  }
  ...
  public addReadOnlyStatistics(
    name: string,
    statistics: Statistics | undefined,
    fractionDigits = FractionDigitsDefault
  ): void {
    if (!statistics || statistics.isEmpty) return;
    ...
    group.addChild(
      new StringProperty({
        ...
        value,
        readonly: true,
        toolTip: `Standard deviation: ${value}`,
      })
    );
  }

  public addReadOnlyRange1(
    name: string,
    range: Range1 | undefined,
    fractionDigits = FractionDigitsDefault
  ): void {
  ...
  public addReadOnlyRange3(
    range: Range3,
    fractionDigits: number = FractionDigitsDefault
  ): void {
  ...
  public addReadOnlyStrings(name: string, ...args: string[]): void {
    ...
    for (const [index, value] of args.entries())
      property.addChild(
        new StringProperty({ name: name + index, value, readonly: true })
      );
  }
}
