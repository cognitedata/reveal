...
4. Make sure to set env variable correctly and run the build,
you will see publicPath printed in console during the build
...
```bash
PUBLIC_PATH=https://static.server/parser-worker/ yarn build
```
</TabItem>
<TabItem value="win">

```bash
set PUBLIC_PATH=https://static.server/parser-worker/ && yarn build
```
</TabItem>
</Tabs>
