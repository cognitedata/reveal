...
describe(IconCollection.name, () => {
  ...
  describe('Cluster creation and children count', () => {
    let mockSceneHandler: SceneHandler;
    let mockEventTrigger: EventTrigger<BeforeSceneRenderedDelegate>;
    let capturedRenderCallback: BeforeSceneRenderedDelegate | undefined;
    let mockRenderer: WebGLRenderer;
    ...
    test('Cluster children array contains correct icons', () => {
      // Group of icons that will be clustered
      const positions = [new Vector3(150, 0, 0), new Vector3(151, 0, 0), new Vector3(150, 1, 0)];
      const collection = new IconCollection(positions, mockSceneHandler, mockEventTrigger);

      expect(capturedRenderCallback).toBeDefined();

      // Camera far from icons
      const camera = createCamera(new Vector3(0, 0, 10), new Vector3(150, 0, 0));
      capturedRenderCallback?.({ frameNumber: 0, renderer: mockRenderer, camera });
      ...
      if (clusterItems.length > 0) {
        ...
        // If clusterIcons is defined, verify count matches clusterSize
        if (cluster.clusterIcons && cluster.clusterIcons.length > 0) {
          expect(cluster.clusterIcons.length).toBe(cluster.clusterSize);

          // Each icon in clusterIcons should have a valid position
          cluster.clusterIcons.forEach(icon => {
            expect(icon.getPosition()).toBeInstanceOf(Vector3);
          });
        }
      }
    ...
    test('ClusteredIcon sizeScale is greater than 1 for clusters', () => {
      const positions = [
        new Vector3(180, 0, 0),
        new Vector3(181, 0, 0),
        new Vector3(180, 1, 0),
      ...
      const camera = createCamera(new Vector3(0, 0, 10), new Vector3(180, 0, 0));
      capturedRenderCallback?.({ frameNumber: 0, renderer: mockRenderer, camera });

      const clusteredIcons = collection.getVisibleClusteredIcons();
      const clusterItems = clusteredIcons.filter((item: ClusteredIcon) => item.isCluster);

      // All clusters should have sizeScale > 1
      clusterItems.forEach((cluster: ClusteredIcon) => {
        expect(cluster.sizeScale).toBeGreaterThan(1);
      });

      // Non-clusters should have sizeScale = 1
      const nonClusterItems = clusteredIcons.filter((item: ClusteredIcon) => !item.isCluster);
      nonClusterItems.forEach((item: ClusteredIcon) => {
        expect(item.sizeScale).toBe(1);
      });
    ...
    test('Total children count across all clusters equals total icon count', () => {
      const positions = [
        // Group 1 - will cluster
        new Vector3(100, 0, 0),
        new Vector3(101, 0, 0),
        new Vector3(100, 1, 0),
        // Group 2 - will cluster
        new Vector3(200, 0, 0),
        new Vector3(201, 0, 0),
        // Close icons - won't cluster
        new Vector3(0, 0, 0),
        new Vector3(1, 0, 0)
      ];
      const collection = new IconCollection(positions, mockSceneHandler, mockEventTrigger);
      ...
      const camera = createCamera(new Vector3(0, 0, 20), new Vector3(100, 0, 0));
      capturedRenderCallback?.({ frameNumber: 0, renderer: mockRenderer, camera });

      const clusteredIcons = collection.getVisibleClusteredIcons();

      // Sum up all clusterSizes - should equal total icon count for visible icons
      let totalCounted = 0;
      clusteredIcons.forEach((item: ClusteredIcon) => {
        totalCounted += item.clusterSize;
      });

      // Total counted should be <= positions.length (some might be culled by frustum)
      expect(totalCounted).toBeLessThanOrEqual(positions.length);
