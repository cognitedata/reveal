...
model
  .iterateNodesByTreeIndex(
    treeIndex => sum = sum + treeIndex
  )
  .then(() => alert('Iteration is done! Sum of all tree indexes is ' + sum));
```
...
let sum = 0;
model
  .iterateSubtreeByTreeIndex(917985, treeIndex => {
    sum = sum + treeIndex;
  })
  .then(() => alert('Iteration is done! Sum of all tree indexes in subtree is ' + sum));
;
```
