...
import { TreeViewInfo, type TreeViewInfoProps } from './tree-view-info';
...
describe(TreeViewInfo.name, () => {
  ...
  test('should click', async () => {
    ...
    const container = renderMe({ node, onClick });
    const icons = getIconsInContainer(container);
  ...
  function renderMe(props: TreeViewInfoProps): HTMLElement {
    return render(<TreeViewInfo {...props} />).container;
  }
});
