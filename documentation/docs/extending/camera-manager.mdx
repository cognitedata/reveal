...
in runtime by calling `setCameraManager` method of `Cognite3DViewer` class. Interface is defined in [here in the API reference](../api/interfaces/cognite_reveal.CameraManager.md).
...
export class CustomCameraManager implements CameraManager {
    ...
    private readonly _stopCameraListener: DebouncedCameraStopEventTrigger;
    ...
    constructor(domElement: HTMLElement, camera: THREE.PerspectiveCamera) {
        ...
        this._stopCameraListener = new DebouncedCameraStopEventTrigger(this);
...
```
As you can see, implementation can be quite simple. There are four things that you should really pay extra attention to:
