...
To implement a custom data source, [DataSource from @cognite/reveal/extensions/datasource](../API Reference.md#interfaces_cognite_reveal_extensions_datasourcedatasourcemd) must be implemented
and provided to `Cognite3DViewer` on construction using the [`customDataSource`-option](../API Reference.md#customdatasource), which in turn
...
[`CdfModelIdentifier`](../API Reference.md#class-cdfmodelidentifier):
```js
...
`getModelUrl()` returns a "base URL" that will be passed to `ModelDataProvider.getBinaryFile()` and `getJsonFile()`. Note that
...
See [API documentation](../API Reference.md#interfaces_cognite_reveal_extensions_datasourcemodeldataprovidermd).
...
See [API documentation](../API Reference.md#interfaces_cognite_reveal_extensions_nodesapiclientmd).
...
notable feature is the [CAD styling API](cad-styling) which in large is backed by `CogniteClient`. To use the styling API with
custom data sources, you will need to [create custom node collections](cad-styling-custom) or [use `TreeIndexNodeCollection`](cad-styling).

Currently there are restrictions in the [the API for saving viewer state](cad-save-viewerstate) causing `Cognite3DViewer.setViewState` not
