...
export class SeismicCubePlaneView extends BaseGroupThreeView {
  ...
  protected get node(): SeismicPlaneNode {
    return super.getNode() as SeismicPlaneNode;
  }

  protected get style(): SeismicRenderStyle {
    return super.getStyle() as SeismicRenderStyle;
  }
  ...
  public /* override */ calculateBoundingBoxCore(): Range3 | undefined {
    ...
    if (!surveyCube) return undefined;
  ...
  public /* override */ beforeRender(): void {
    ...
    const uniqueId = seismicCubeNode
      ? seismicCubeNode.uniqueId
      : UniqueId.empty;
    ...
    if (!uniqueId.equals(this._uniqueId)) {
      ...
      node.perpendicularIndex !== this._index ||
      node.perpendicularAxis !== this._axis
    ) {
      this._index = node.perpendicularIndex;
      this._axis = node.perpendicularAxis;
      this.updateTextureCoords(
  ...
  protected /* override */ createObject3DCore(): THREE.Object3D | null {
    ...
    const { surveyNode } = node;
    if (!surveyNode) return parent;

    const { seismicCubeNode } = this;

    const style = seismicCubeNode
      ? (seismicCubeNode.getRenderStyle(this.targetId) as SeismicRenderStyle)
      : null;
    const { surveyCube } = node;
    if (!surveyCube) return parent;

    const solid = this.createSolid(node, surveyCube, style);
    if (solid) parent.add(solid);
  ...
  private get seismicCubeNode(): SeismicCubeNode | null {
    const { node } = this;
    const { surveyNode } = node;
    if (!surveyNode) return null;

    for (const seismicCubeNode of surveyNode.getDescendantsByType(
      SeismicCubeNode
    )) {
      const view = seismicCubeNode.getViewByTarget(this.renderTarget);
      if (view) return seismicCubeNode;
    }
    return null;
  }
  ...
  private createSolid(
    ...
    surveyCube: RegularGrid3,
    style: SeismicRenderStyle | null
  ): THREE.Object3D | null {
    const cells = node.createCells(false);
    if (cells.length < 2) return null;

    const { unrotatedNormal } = node;
    if (node.perpendicularAxis === 0) unrotatedNormal.negate();
  ...
  private updateTextureCoords(
    ...
    if (!this.hasTarget) return; // This can happen when turning on or off
    ...
    if (!cube) {
      this.move(mesh, surveyCube, minCell);
      for (let i = uv.length - 1; i >= 0; i--) uv[i] = 0;
      return;
    }
    if (!cube.client) return;

    this.move(mesh, surveyCube, minCell);

    if (inDragging) this._timeStamp++;
    const timeStamp = this._timeStamp;
    if (inDragging && timeStamp < this._timeStamp) return;
    ...
    (async () => {
      ...
      while (true) {
        if (inDragging && timeStamp < this._timeStamp) break;

        if (minIndex > cells.length - 1) break;
      ...
      if (seismicCubeNode) this.updateTextureMap(parent, seismicCubeNode);
  ...
  private updateTextureMap(
    ...
    const texture = TextureKit.create1D(
      ColorMaps.get(seismicCubeNode.colorMap)
    );
