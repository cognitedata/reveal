...
export abstract class BaseGroupThreeView extends BaseThreeView {
  ...
  protected get object3D(): THREE.Object3D | null {
    if (this.mustTouch()) this.touchPart();
    if (!this._object3D) this.makeObject();
  ...
  protected /* override */ updateCore(args: NodeEventArgs): void {
    ...
    if (args.isChanged(Changes.geometry)) this.touch();
  }

  protected /* override */ clearMemoryCore(): void {
    if (!this.isVisible) this.touch(); // Remove the group from the memory
  }

  protected /* override */ onShowCore(): void {
    super.onShowCore();
    // Create the group and add it to the scene
    if (this._object3D) this._object3D.visible = true;
  }
  ...
  public /* override */ calculateBoundingBoxCore(): Range3 | undefined {
    if (!this.object3D) return undefined;

    const boundingBox = BoundingBoxKit.getBoundingBox(
      this.object3D,
      this.transformer
    );
  ...
  protected /* virtual */ mustTouch(): boolean {
    return false;
  }

  protected /* virtual */ touchPart(): void {
    this.touch();
  }

  public /* virtual */ touch(): void {
    const { scene } = this;
    if (this._object3D) scene.remove(this._object3D);
