...
Notice that you need to have `pointSizeType` set to `Fixed` for `pointSize` to work,
because by default `pointSize` is calculated automatically.

```js runnable
// import { PotreePointSizeType } from '@cognite/reveal'
